...
router.post('/registration', 
    [
        ...
        check('password', 'Неверный пароль').isLength({min: 2, max: 20}),
        check('name', 'Имя обязательно для заполнения').not().isEmpty(),
        check('id', 'Персональный код обязательно для заполнения').not().isEmpty(),
    ...
    try {
        ...
        if(!errors.isEmpty()){
            console.log(errors.array()); // Вывести ошибки в консоль
            return res.status(400).json({message: "Неверный запрос", errors})
        }

        const {phone, password, name, surname, id} = req.body;
        ...
        const user = new User({phone, password: hashPassword, name, surname, userID: id, createdAt: new Date() })
        await user.save()
...
router.post('/update-id', async (req, res) => {
    try {
        // Получаем токен из заголовка запроса или из cookies
        const token = req.headers.authorization.split(' ')[1] || req.cookies.token;
        ...
        // Обновляем персональный код пользователя
